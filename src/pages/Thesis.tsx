...
const layerDetails = [
  {
    ...
    activeColor: "border-orange-500/60 glow-orange",
  ...
  },
  {
    ...
    activeColor: "border-orange-400/60 glow-orange",
    items: ["CDC Streams", "Batch ETL", "dbt Models", "Quality Gates"],
    icon: GitBranch,
    detail: {
      title: "Transformation & Modeling",
      description:
        "Raw data lands in a bronze layer. dbt models progressively clean, deduplicate, and enrich through silver to gold. Every model is tested, documented, and version-controlled. Quality gates validate schema, freshness, and statistical expectations at every layer transition.",
  ...
  },
  {
    ...
    activeColor: "border-primary/60 glow-orange",
  ...
  },
  {
    ...
    activeColor: "border-accent/60 glow-orange",
  ...
  return (
    <div className="relative overflow-hidden">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 relative">
        ...
        <svg
          className="absolute inset-0 w-full h-full pointer-events-none hidden lg:block z-0 overflow-hidden"
            ...
            >
              ...
              >
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <div
                    className={`w-7 h-7 rounded-lg bg-gradient-to-br ${layer.color} flex items-center justify-center`}
                  >
  ...
  const stages = [
    {
      level: "Level 1",
      label: "Reactive",
      desc: "Manual reports, Excel-driven, no single source of truth",
      status: "outdated" as const,
    },
    {
      level: "Level 2",
      label: "Managed",
      desc: "Centralized warehouse, scheduled dashboards, basic governance",
      status: "baseline" as const,
    },
    {
      level: "Level 3",
      label: "Proactive",
      desc: "Real-time pipelines, data quality gates, observability built-in",
      status: "competitive" as const,
    },
    {
      level: "Level 4",
      label: "Predictive",
      desc: "ML models in production, forecasting, anomaly detection, feature stores",
      status: "advanced" as const,
    },
    {
      level: "Level 5",
      label: "Autonomous",
      desc: "AI agents acting on data, self-healing pipelines, decision systems",
      ...
      <div className="space-y-4">
        {stages.map((stage, i) => (
          ...
          >
            {/* Dot on timeline */}
            <div
              className={`hidden sm:flex relative z-10 w-12 h-12 rounded-xl glass-panel items-center justify-center shrink-0 ${i === stages.length - 1 ? "border-accent/40" : ""}`}
            >
              <span className="font-mono text-xs font-bold text-primary">{i + 1}</span>
            </div>

            <div
              className={`flex-1 glass-panel rounded-xl p-4 sm:p-5 ${i === stages.length - 1 ? "border-accent/20 glow-orange" : ""}`}
              style={
                i === stages.length - 1
                  ? {
                      background:
                        "linear-gradient(135deg, hsl(var(--glass-strong-bg) / var(--glass-strong-opacity)), hsl(var(--orange-start) / 0.05))",
                    }
                  : undefined
              }
            >
              <div className="flex items-center gap-3 mb-2">
                ...
                <span
                  className={`text-[9px] font-mono uppercase tracking-wider px-2 py-0.5 rounded-full border ${statusColors[stage.status]}`}
                >
                  {stage.status}
                </span>
              </div>
...
const AutomationFlow = () => {
  const steps = [
    ...
    { icon: Brain, label: "Analyze", desc: "AI agent correlates with upstream schema change in CRM sync" },
    {
      icon: AlertTriangle,
      label: "Diagnose",
      desc: "Root cause: new field mapping in Salesforce broke join condition",
    },
...
const BeforeAfter = () => {
  const comparisons = [
    {
      dimension: "Reporting",
      before: "Monthly static PDFs, 3-week lag",
      after: "Real-time dashboards, sub-second refresh",
    },
    {
      dimension: "Data Quality",
      before: "Discovered by end-users in production",
      after: "Caught at ingestion with automated gates",
    },
    {
    ...
    {
      dimension: "ML / AI",
      before: "Jupyter notebooks on a laptop",
      after: "Production MLOps with monitoring & retraining",
    },
    {
    ...
    },
    {
      ...
      before: "War room, 4+ hours to diagnose",
      after: "Auto-detected, root-caused, resolved in minutes",
    },
  ];
        ...
        <div className="pb-24">
          <div className="max-w-6xl mx-auto px-6 lg:px-8">
            ...
            <section id="the-problem" className="relative mb-24 scroll-mt-24">
              ...
              {/* Before / After */}
              <motion.div {...fadeUp}>
                <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground/50 mb-4">
                  The Transformation
                </p>
                <BeforeAfter />
              </motion.div>
            ...
            <section id="enterprise-principles" className="relative mb-24 scroll-mt-24">
              ...
              <motion.div {...fadeUp} className="max-w-3xl mb-10">
                ...
                <h2 className="text-2xl sm:text-3xl font-bold text-foreground mb-4">
                  12 Principles of <span className="text-gradient-orange">AI-Ready Enterprise Data</span> Architecture
                </h2>
                <p className="text-muted-foreground leading-relaxed">
                  These aren't aspirational values. They are hard engineering constraints we enforce in every
                  engagement. Each principle has concrete implementation patterns, automated checks, and measurable
                  outcomes.
                </p>
              </motion.div>
                      ...
                      <div className="space-y-2">
                        {item.examples.map((ex, j) => (
                          <div key={j} className="flex items-start gap-2 pt-2 border-t border-border/20">
                            ...
                            <p className="text-[10px] font-mono text-muted-foreground/60 italic leading-relaxed">
                              {ex}
                            </p>
                          </div>
                        ))}
